...
object MVPScreen {
    ...
    fun build(player: Player): YamlConfiguration {
        return ScreenUI("mvp").apply {
            ...
            addComponent(
                TextureComp("mvp_icon", "ui/support/3.png")
            ...
            if (player.hasPermission("svp")) {
                addComponent(LabelComp("mvp_buy_bg_label", "")
                    .setTexts("func.PlaceholderAPI_Get('ksupporter_mvp') == true ? '' : '&e㊉ 300,000 神石'")
                    .setXY("mvp_buy_bg.x+(mvp_buy.width-mvp_buy_bg_label.width)/2+2", "mvp_buy_bg.y+5")
                )
            } else {
                addComponent(LabelComp("mvp_buy_bg_label", "")
                    .setTexts("func.PlaceholderAPI_Get('ksupporter_mvp') == true ? '' : '&e㊉ 400,000 神石'")
                    .setXY("mvp_buy_bg.x+(mvp_buy.width-mvp_buy_bg_label.width)/2+2", "mvp_buy_bg.y+5")
                )
            }
            if (player.hasPermission("mvp")) {
                addComponent(TextureComp("mvp_cant_buy", "ui/common/button_normal_g.png")
                    .setText("&f&l已购买")
